...
async function handleRegister() {
  ...
  try {
    await $fetch(`${apiBase}/register`, {
      method: 'POST',
      body: { email: regEmail.value, username: regUsername.value, password: regPassword.value },
    })
    const data = await $fetch<{ token: string }>(`${apiBase}/login`, {
      method: 'POST',
...
<template>
  <div class="min-h-screen flex items-center justify-center bg-[#1a0e09] px-4 py-10">
    <div
      class="w-full max-w-sm rounded-2xl overflow-hidden transition-colors duration-300 shadow-2xl"
      :class="isDark ? 'bg-secondaire text-primaire' : 'bg-primaire text-secondaire'"
    >
      <!-- Logo -->
      ...
      <!-- Onglets -->
      <div class="flex border-b" :class="isDark ? 'border-accent/40' : 'border-accent/30'">
        <button
          class="flex-1 py-3 text-sm font-semibold transition-colors relative"
          :class="tab === 'connexion'
            ? 'text-cta'
            : isDark ? 'text-primaire/50' : 'text-secondaire/50'"
          @click="tab = 'connexion'"
        >
          Connexion
          <span
      ...
      <form v-if="tab === 'connexion'" class="px-8 py-6 space-y-4" @submit.prevent="handleLogin">
        <div>
          <label class="block text-xs font-semibold mb-1 text-primaire/80">Adresse mail</label>
          <input
            v-model="loginEmail"
            type="email"
        ...
        <div>
          <div class="flex justify-between items-center mb-1">
            <label class="text-xs font-semibold text-primaire/80">Mot de passe</label>
          ...
          <div class="relative">
            ...
            >
              <svg v-if="!showLoginPassword" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="flex items-center gap-3 py-1">
          ...
          <span class="text-xs font-semibold tracking-widest text-primaire/40">OU VIA CANAL SÉCURISÉ</span>
          <div class="flex-1 h-px" :class="'bg-primaire/20'" />
        </div>
      ...
      <form v-else class="px-8 py-6 space-y-4" @submit.prevent="handleRegister">
        <div>
          <label class="block text-xs font-semibold mb-1 text-secondaire/70">Pseudo</label>
          <input
            v-model="regUsername"
            type="text"
          ...
          />
          <p v-if="regErrors.username" class="text-alerte text-xs mt-1">{{ regErrors.username }}</p>
        </div>

        <div>
          <label class="block text-xs font-semibold mb-1 text-secondaire/70">Votre adresse mail</label>
          <input
            v-model="regEmail"
            type="email"
          ...
          />
          <p v-if="regErrors.email" class="text-alerte text-xs mt-1">{{ regErrors.email }}</p>
        </div>

        <div>
          <label class="block text-xs font-semibold mb-1 text-secondaire/70">Définir le mot de passe</label>
          <div class="relative">
            <input
              v-model="regPassword"
            ...
            >
              <svg v-if="!showRegPassword" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.477 0 8.268 2.943 9.542 7-1.274 4.057-5.065 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
              </svg>
              <svg v-else class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <p v-if="regErrors.password" class="text-alerte text-xs mt-1">{{ regErrors.password }}</p>
        </div>

        <label class="flex items-start gap-2 cursor-pointer">
          <input v-model="regAccept" type="checkbox" class="mt-0.5 accent-cta" />
          <span class="text-xs text-secondaire/70 leading-snug">
            J'accepte le Code de Conduite Tactique et j'autorise Crooak à gérer mes données de jeu.
          </span>
        </label>
        <p v-if="regErrors.accept" class="text-alerte text-xs">{{ regErrors.accept }}</p>
        <p v-if="regErrors.general" class="text-alerte text-xs">{{ regErrors.general }}</p>
        ...
        <div class="flex items-center gap-3 py-1">
          ...
          <span class="text-xs font-semibold tracking-widest text-secondaire/40">OU VIA CANAL SÉCURISÉ</span>
          <div class="flex-1 h-px bg-secondaire/20" />
        </div>
