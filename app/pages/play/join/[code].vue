...
const status = ref<"loading" | "confirm" | "joining" | "done" | "error">(
  "loading",
);
...
<template>
  <div
    ...
    "
  >
    <img src="/images/logo_crooak.png" alt="CROOAK" class="h-28 w-auto" />

    <template v-if="status === 'confirm' || status === 'joining'">
      <p class="text-sm uppercase tracking-[0.3em] text-primaire/60">
    ...
    <template v-else-if="status === 'done'">
      ...
      <p class="text-primaire/70">
        Tu es dans la partie.<br />Tu peux fermer cette page, bonne chance !
      </p>
      <NuxtLink
        :to="`/`"
